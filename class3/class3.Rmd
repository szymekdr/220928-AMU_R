...
**Zadanie** Opierając się na prostej regresji skonstruuj model, w którym na `tarsus` potencjalnie wpływają: `hatchdate`, `sex`, `habitat`, interakcja `habitat` x `hatchdate`, interakcja `sex` x `hatchdate`. Zapisz go w nowym obiekcie - czy którykolwiek czynnik modelu jest istotny statystycznie?
...
**Zadanie** Porównaj uzyskany output z wynikiem użycia w podobny sposób funkcji `aov()`.
